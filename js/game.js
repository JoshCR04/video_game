...
gameScene.create = function () {
    ...
    // Se pone el pasto
    let pastoSuperior = this.add
        .image(0, fondo.height, "background_superior")
        .setOrigin(0, 1);
...
gameScene.handleGameOver = function () {
    ...
    // Crear un fondo negro que cubra toda la pantalla
    ...
    // Crear el texto centrado en la pantalla
    let text = this.add.text(this.cameras.main.centerX, this.cameras.main.centerY, "You Lost!", textStyle).setOrigin(0.5).setDepth(2);

    // Ajustar la cámara para que muestre toda la pantalla
...
gameScene.update = function () {
    ...
    this.enemiesGroup.getChildren().forEach((enemy) => {
        // Verificar si el enemigo está bloqueado y cambiar dirección
        if (enemy.body.blocked.left || enemy.body.blocked.right) {
            enemy.patrolDirection = !enemy.patrolDirection; // Cambiar dirección al chocar
        }

        // Mover al enemigo en la dirección de patrullaje
        enemy.body.setVelocityX(enemy.patrolDirection ? 150 : -150); // 150 si patrulla a la derecha, -150 si a la izquierda

        // Girar el enemigo según la dirección
        enemy.setFlipX(enemy.patrolDirection); // Mirar a la derecha si patrulla a la derecha

        // Verificar si el enemigo ha alcanzado el límite de patrullaje
        if (enemy.x > enemy.startX + enemy.patrolDistance) {
            enemy.patrolDirection = false; // Cambiar dirección a izquierda
        } else if (enemy.x < enemy.startX - enemy.patrolDistance) {
            enemy.patrolDirection = true; // Cambiar dirección a derecha
        }
    });
